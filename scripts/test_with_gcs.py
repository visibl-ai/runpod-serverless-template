...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Test RunPod endpoint with GCS signed URL"
    )
    ...
    parser.add_argument(
        "--json-input", type=str, help="Path to JSON file containing input data"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="GCS signed URL to upload results to",
    )
...
def run_test(args):
    ...
    if not api_key:
        print(
            "Error: RunPod API key is required. Provide it with --api-key or set RUNPOD_API_KEY environment variable."
        )
        sys.exit(1)
    ...
    if not endpoint_id:
        print(
            "Error: RunPod endpoint ID is required. Provide it with --endpoint-id or set RUNPOD_ENDPOINT_ID environment variable."
        )
        sys.exit(1)
    ...
    payload = {"input": input_data, "gcs_signed_url": signed_url}
    ...
    try:
        ...
        if "id" in response_data:
            ...
            print(
                "This is an asynchronous job. You'll need to check the status separately."
            )
        ...
        print(
            "You can check the uploaded content through the Google Cloud Console or other GCS tools."
        )

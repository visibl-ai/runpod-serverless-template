...
def parse_args():
    ...
    parser.add_argument(
        "--signed-url", type=str, help="GCS signed URL for testing", default=""
    )
...
def test_text():
    ...
    test_event = {"input": {"text": "This is a sample text for testing."}}
...
def test_with_callback():
    ...
    test_event = {
        "input": {"text": "This is a test with callback URL."},
...
def test_with_gcs_signed_url(signed_url=None):
    ...
    actual_signed_url = (
        signed_url
        or "https://storage.googleapis.com/example-bucket/results.json?X-Goog-Signature=example"
    )

    test_event = {
        "input": {"text": "This is a test with GCS signed URL."},
    ...
    redacted_event["gcs_signed_url"] = (
        "(real signed url)" if signed_url else actual_signed_url
    )
...
def test_with_callback_and_gcs(signed_url=None):
    ...
    test_event = {
        "input": {"text": "This is a test with both callback and GCS signed URL."},
        "callback_url": "https://example.com/callback",
        "gcs_signed_url": actual_signed_url,
        "id": "test-job-789",
    }
